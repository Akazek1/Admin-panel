...
// Mirrors app/admin/users/page.tsx (the "Individuals" screen) exactly in
// structure and behaviour — same stat cards, filter bar, bulk-select header,
// sortable table, pagination shell, row-click-to-detail. What differs is only
// what a company account actually is: no personal "Provider" concept (every
// company account already lists services), and "Verification" here means the
// BUSINESS was checked (Organization.verified, admin-actioned) rather than a
// personal ID check. Row click reuses the SAME /admin/users/:id detail page —
// a company account is a real User row (accountType COMPANY), so every
// existing edit/ban/document/history feature already applies to it unchanged.
...
import { getAllUsers, updateOrganization, verifyOrganization, User } from "@/lib/api"
...
type SortKey = "company" | "verification" | "status" | "reports" | "bookings" | "lastActive" | "joined"
type SortDirection = "asc" | "desc"

function companyName(user: User) {
  return user.company?.name || user.firstName || user.phoneNumber || "Unnamed Company"
}
...
function isVerified(user: User) {
  return user.company?.verified ?? false
}

function bookingCount(user: User) {
  return (
    (user._count?.bookingsAsWorker ?? 0) + (user._count?.bookingsAsEmployer ?? 0) ||
...
function sortValue(user: User, sortKey: SortKey) {
  if (sortKey === "company") return companyName(user).toLowerCase()
...
export default function CompaniesPage() {
  ...
  // Same query key as the Individuals page — companies and individuals are
  // both just `accountType`-filtered views over one users list, so this reuses
  // that cache instead of firing a second identical request.
  ...
  const verifyMutation = useMutation({
    mutationFn: (companyId: string) => verifyOrganization(companyId),
    onSuccess: () => {
  ...
  const unverifyMutation = useMutation({
    mutationFn: (companyId: string) => updateOrganization(companyId, { verified: false }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-users"] })
      queryClient.invalidateQueries({ queryKey: ["admin-organizations"] })
      toast({ title: "Company unverified" })
    },
  })

  const companies = useMemo(() => (users ?? []).filter(isCompanyAccount), [users])

  const filteredUsers = useMemo(() => {
    const normalizedSearch = deferredSearchTerm.trim().toLowerCase()
    return companies.filter((user) => {
      const haystack = `${companyName(user)} ${user.phoneNumber || ""} ${user.email || ""} ${user.id}`.toLowerCase()
  ...
  }, [companies, deferredSearchTerm, verificationFilter, statusFilter])

  const sortedUsers = useMemo(
    () => [...filteredUsers].sort((a, b) => compareUsers(a, b, sortKey, sortDirection)),
  ...
  const stats = useMemo(() => {
    const verified = companies.filter(isVerified).length
    const banned = companies.filter((user) => user.isBanned).length
    const reports = companies.reduce((sum, user) => sum + reportCount(user), 0)
    return {
      total: companies.length,
      verified,
      pending: Math.max(0, companies.length - verified),
      banned,
      reports,
    }
  }, [companies])
  ...
  useEffect(() => {
    setCurrentPage(1)
  }, [deferredSearchTerm, verificationFilter, statusFilter, pageSize])
  ...
  const handleSort = (nextSortKey: SortKey) => {
    ...
    setSortDirection(nextSortKey === "company" ? "asc" : "desc")
  }
  ...
  return (
    <div className="min-h-screen bg-[#101211] p-6">
      <div className="mx-auto max-w-[1500px] space-y-5">
        <AdminPageHeader title="Companies" description="Manage and monitor service-company provider accounts.">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          <AdminStatCard title="Total Companies" value={stats.total.toLocaleString()} description="Provider accounts" icon={Building2} tone="green" />
          <AdminStatCard title="Pending Verification" value={stats.pending.toLocaleString()} description="Needs review" icon={CheckCircle2} tone="amber" />
        ...
        <div className="rounded-lg border border-white/5 bg-card/70 p-4 shadow-sm shadow-black/10">
          <div className="grid gap-3 xl:grid-cols-[minmax(260px,1fr)_180px_160px_auto]">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
              <Input
                placeholder="Search by company name, email, phone or ID..."
        ...
        <div className="overflow-hidden rounded-lg border border-white/5 bg-card/70 shadow-sm shadow-black/10">
          <div className="flex flex-col gap-3 border-b border-white/5 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap items-center gap-3">
              <Checkbox checked={allVisibleSelected} onCheckedChange={(value) => toggleVisibleUsers(!!value)} aria-label="Select visible companies" />
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Select value={`${sortKey}:${sortDirection}`} onValueChange={handleSortSelect}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="company:asc">Sort by: Name A-Z</SelectItem>
                  <SelectItem value="company:desc">Sort by: Name Z-A</SelectItem>
          ...
          {isLoading ? (
            ...
            <div className="p-5">
              <EmptyState title="No companies found" description="Adjust the filters or search term to widen the result set." icon={Building2} />
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow className="border-white/5 hover:bg-transparent">
                  ...
                  <TableHead>
                    <SortableHead label="Company" sortKey="company" activeSortKey={sortKey} direction={sortDirection} onSort={handleSort} />
                  </TableHead>
                  <TableHead>
                    ...
                    <TableRow key={user.id} className="cursor-pointer border-white/5 hover:bg-white/[0.03]" onClick={() => router.push(`/admin/users/${user.id}`)}>
                      ...
                      <TableCell>
                        <div className="flex min-w-[240px] items-center gap-3">
                          {user.company?.logoUrl || user.profilePicture ? (
                            <img src={user.company?.logoUrl || user.profilePicture || ""} alt="" className="h-10 w-10 rounded-full object-cover ring-1 ring-white/10" />
                          ) : (
                          ...
                          <div className="min-w-0">
                            <p className="truncate text-sm font-medium text-foreground">{companyName(user)}</p>
                      ...
                      <TableCell onClick={(event) => event.stopPropagation()}>
                        <button
                          type="button"
                          disabled={!user.company?.id || togglePending}
                          onClick={() => {
                            if (!user.company?.id) return
                            if (verified) unverifyMutation.mutate(user.company.id)
                            else verifyMutation.mutate(user.company.id)
                          }}
                        ...
                        >
                          <Badge
                            ...
                            className={cn(
                              "cursor-pointer",
                              verified ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-300" : "border-amber-500/20 bg-amber-500/10 text-amber-300",
          ...
          <div className="flex flex-col gap-3 border-t border-white/5 px-4 py-3 md:flex-row md:items-center md:justify-between">
            <p className="text-sm text-muted-foreground">
              Showing {firstItem} to {lastItem} of {sortedUsers.length.toLocaleString()} companies
            </p>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex h-9 min-w-9 items-center justify-center rounded-md border border-primary/50 px-3 text-sm text-primary">{currentPage}</div>
      ...
      <CreateAccountDialog open={createOpen} onOpenChange={setCreateOpen} defaultPersona="COMPANY" />
    </div>
  )
}
